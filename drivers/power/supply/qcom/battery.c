...
static int cp_get_parallel_mode(struct pl_data *chip, int mode)
{
	...
	case PARALLEL_OUTPUT_MODE:
		...
		break;
	default:
		pr_debug("Invalid mode request %d\n", mode);
		break;
	}

	if (rc < 0)
		pr_debug("Failed to read CP topology for mode=%d rc=%d\n",
				mode, rc);
...
static int get_adapter_icl_based_ilim(struct pl_data *chip)
{
	...
	if (rc < 0)
		pr_debug("Failed to read PD_ACTIVE status rc=%d\n",
				rc);
...
static void get_main_fcc_config(struct pl_data *chip, int *total_fcc)
{
	...
	if (rc < 0) {
		pr_debug("Couldn't get switcher enable status, rc=%d\n", rc);
		goto out;
	}
			...
			int parallel_fcc_ua)
{
	...
	if (!chip->chg_param->fcc_step_size_ua) {
		pr_debug("Invalid fcc stepper step size, value 0\n");
		return;
	}
	...
	while (true) {
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER) {
			fcc_ua = get_client_vote(chip->fcc_votable,
					TAPER_STEPPER_VOTER);
			if (fcc_ua < 0) {
				pr_debug("Couldn't get fcc, exiting taper work\n");
				goto done;
			}
			fcc_ua -= TAPER_REDUCTION_UA;
			if (fcc_ua < 0) {
				pr_debug("Can't reduce FCC any more\n");
				goto done;
			}
	...
	if (cp_fcc_ua > 0) {
		if (chip->cp_master_psy) {
			rc = power_supply_get_property(chip->cp_master_psy,
					POWER_SUPPLY_PROP_MIN_ICL, &pval);
			if (rc < 0)
				pr_debug("Couldn't get MIN ICL threshold rc=%d\n",
									rc);
		}
...
static void fcc_stepper_work(struct work_struct *work)
{
	...
	if (chip->dc_psy) {
		rc = power_supply_get_property(chip->dc_psy,
				POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0)
			pr_debug("Couldn't get DC Present status, rc=%d\n", rc);
			...
			int fv_uv, const char *client)
{
	...
	if ((chip->float_voltage_uv < fv_uv) && is_batt_available(chip)) {
		...
		if (rc < 0) {
			pr_debug("Couldn't get battery status rc=%d\n", rc);
		} else {
			if (pval.intval == POWER_SUPPLY_STATUS_FULL) {
				...
				rc = power_supply_set_property(chip->batt_psy,
					...
					&pval);
				if (rc < 0)
					pr_debug("Couldn't set force recharge rc=%d\n",
							rc);
			}
		}
	}
	...
	if (cp_get_parallel_mode(chip, PARALLEL_INPUT_MODE)
		...
		if (chip->dc_psy) {
			rc = power_supply_get_property(chip->dc_psy,
					POWER_SUPPLY_PROP_PRESENT, &pval);
			if (rc < 0) {
				pr_debug("Couldn't get DC PRESENT rc=%d\n", rc);
				return rc;
			}
...
static bool is_parallel_available(struct pl_data *chip)
{
	...
	if (rc < 0) {
		pr_debug("Couldn't get parallel batfet mode rc=%d\n",
				rc);
		return false;
	}
...
static void handle_usb_change(struct pl_data *chip)
{
	...
	rc = power_supply_get_property(chip->usb_psy,
			POWER_SUPPLY_PROP_PRESENT, &pval);
	if (rc < 0) {
		pr_debug("Couldn't get present from USB rc=%d\n", rc);
		return;
	}
...
int qcom_batt_init(struct charger_param *chg_param)
{
	...
	if (!chg_param) {
		pr_debug("invalid charger parameter\n");
		return -EINVAL;
	}
