...
{
	unsigned int cachestrategy = EROFS_I_SB(fe->inode)->cache_strategy;
				       ...
				       int err)
{
	...
	if (!be->decompressed_pages)
		be->decompressed_pages =
			erofs_calloc_nofail(be->nr_pages, sizeof(struct page *));
	if (!be->compressed_pages)
		be->compressed_pages =
			erofs_calloc_nofail(pclusterpages, sizeof(struct page *));
				 ...
				 bool *force_fg)
{
	...
	do {
		...
		do {
			...
			page = pickup_page_for_submission(pcl, i++, pagepool,mc);
			if (!page)
				continue;
